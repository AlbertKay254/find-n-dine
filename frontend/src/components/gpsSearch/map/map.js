...
export default class map extends React.Component {
      ...
      <div className="map-header max-width">
        ...
        >
          ...
          <div className="buttons">
            <button className="icon-container" onClick={handleSearch}>
              <SearchIcon />
              Search
            </button>
            <button className="icon-container" onClick={resetAll}>
  ...
  maps = () => {
    ...
    const center = { lat: -1.2769476811310503, lng: 36.81720222954108 };

    return (
      <div
        className="map max-width"
        style={{ height: "80vh", borderRadius: "22px" }}
      >
        <GoogleMapReact
          bootstrapURLKeys={{
            key: "AIzaSyDSMDqx6HgCylt6KnlHdf1-QPeKY7GW5kU",
          }}
          defaultCenter={{
          ...
          {this.state.rests.map((rest) => {
            return (
              <h1
                ...
                lng={rest.longitude}
                onClick={() => {
                  this.setState({ selectedRestId: rest.id });
                }}
              >
                Hwy
              </h1>
